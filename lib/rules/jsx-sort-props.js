...
const messages = {
  ...
  sortPropsByAlpha: 'Props should be sorted alphabetically LOLOLOLOLOL',
};
...
function contextCompare(a, b, options) {
  ...
  if (isSetFunction(aProp)) {
    aProp = aProp.substring(3);
  }

  if (isSetFunction(bProp)) {
    bProp = bProp.substring(3);
  }
...
function generateFixerFunction(node, context, reservedList) {
  ...
  return function fixFunction(fixer) {
    ...
    fixers.forEach((fix) => {
      source = `${source.slice(0, fix.range[0])}${fix.text}${source.slice(
        fix.range[1],
      )}`;
    });
...
module.exports = {
  ...
  create(context) {
    ...
    const shorthandLast = configuration.shorthandLast || false;
    const multiline = configuration.multiline || 'ignore';
    const groupSetterWithGetter = configuration.groupSetterWithGetter || true;
    const noSortAlphabetically = configuration.noSortAlphabetically || false;
    const reservedFirst = configuration.reservedFirst || false;
    ...
    return {
      ...
      JSXOpeningElement(node) {
        ...
        const nodeReservedList =
          reservedFirst && !jsxUtil.isDOMComponent(node)
            ? reservedList.filter((prop) => prop !== 'dangerouslySetInnerHTML')
            : reservedList;

        node.attributes.reduce((previous, current, idx, attrs) => {
          if (current.type === 'JSXSpreadAttribute') {
            return attrs[idx + 1];
          }

          let previousPropName = propName(previous);
          let currentPropName = propName(current);
          ...
          if (groupSetterWithGetter) {
            if (isSetFunction(previousPropName)) {
              console.log('IT IS A SETTER (prev)');
              previousPropName = previousPropName.substring(3).toLowerCase();
            }
            if (isSetFunction(currentPropName)) {
              console.log('IT IS A SETTER (curr)');
              currentPropName = currentPropName.substring(3).toLowerCase();
            }
          }

          if (reservedFirst) {
            if (reservedFirstError) {
              reservedFirstError(current);
              return previous;
            }
          ...
          const previousIsMultiline = isMultilineProp(previous);
          const currentIsMultiline = isMultilineProp(current);
          if (multiline === 'first') {
          ...
          if (groupSetterWithGetter) {
            // const previousIsSetter = previousPropName.startsWith('set');

            const currentIsSetter = isSetFunction(propName(current));
            // if (previousIsSetter) {
            //   const previousAsLowerCaseGetterName = previousPropName
            //     .replace('set', '')
            //     .toLowerCase();
            //   const currentLowerCase = currentPropName
            //     .replace('get', '')
            //     .toLowerCase();
            //   if (previousAsLowerCaseGetterName === currentLowerCase) {
            //     return current;
            //   }
            // }
            if (currentIsSetter) {
              const currentAsLowerCaseGetterName =
                currentPropName.toLowerCase();
              const previousLowerCase = previousPropName.toLowerCase();
              if (currentAsLowerCaseGetterName === previousLowerCase) {
                return previous;
              }
            }
          }
          ...
          if (
            ...
            console.log(
              'previousPropName, currentPropName, propName(previous), propName(current)',
              previousPropName,
              currentPropName,
              propName(previous),
              propName(current),
